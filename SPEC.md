...
- Mid workout persistence: the in progress session is saved locally, so closing, backgrounding, or reloading the app never loses a logged set. Sets logged with no connection are held on the device and sync as soon as it returns. One case is narrower than that sounds: reloading the page while offline returns the app shell rather than the workout screen, because the offline cache covers the shell and not individual session routes. Nothing is lost when that happens, and the workout is there again once a connection is. This is local resilience only, not full offline sync (see Deferred to v2).
