...
// order, which is irrelevant here. Bodyweight moves up so it lands inside the
// roughly four chips that fit a 390px viewport without swiping, since it is
// arguably the most useful value for anyone training at home. Other stays
// last: it is the catch-all, and it means unknown rather than a category,
// which is why the routine builder must never read it as one.
